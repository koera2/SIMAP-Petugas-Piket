<?php  
	...
	if (isset($_GET['rayonM'])) {
		...
		$nama = "Laporan Izin Siswa Rayon ".$rayon['rayon']." Bulan ".$bulan." ".$tahun.".pdf";
	}elseif (isset($_GET['rayon'])) {
		$where = "WHERE kode_rayon = '$kd_rayon'";
		$sum = "nis,nama,rombel,kode_rayon,COUNT(jenis_izin) as jumlah_izin";
		$data = $aksi->tampil_sum($sum,$table,$where,"GROUP BY nis ORDER BY nis ASC");
		$nama = "Laporan Izin Siswa Rayon ".$rayon['rayon']." Periode ".$dari." sampai ".$sampai.".pdf";
		...
		$sum = "nis,nama,rombel,kode_rayon,COUNT(jenis_izin) as jumlah_izin";
		$data = $aksi->tampil_sum($sum,$table,$where,"GROUP BY nis ORDER BY nis ASC");
		$nama = "Laporan Izin Siswa Rayon ".$rayon['rayon']." Semester ".$semester." tp ".$tpel.".pdf";
	}
