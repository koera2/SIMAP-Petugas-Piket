<?php  
	...
	$judul = "LAPORAN KINERJA SISWA SMK WIKRAMA KOTA BOGOR RAYON ".strtoupper($rayon['rayon']);
	...
	if (isset($_GET['rayonM'])) {
		...
		$where = "WHERE kode_rayon = '$kd_rayon' AND MONTH(tgl_kejadian)='$bulan' AND YEAR(tgl_kejadian)='$tahun'";
		...
		$nama = "Laporan Kinerja Siswa Rayon ".$rayon['rayon']." Bulan ".$bulan." ".$tahun.".pdf";
	}elseif (isset($_GET['rayon'])) {
		$where = "WHERE kode_rayon = '$kd_rayon'";
		$sum = "nis,nama,rombel,kode_rayon,SUM(skor_r) as skor_reward, SUM(skor_p) as skor_punishment";
		$nama = "Laporan Kinerja Siswa Rayon ".$rayon['rayon'].".pdf";
		$data = $aksi->tampil_sum($sum,$table,$where,"GROUP BY nis ORDER BY nis ASC");
	}elseif (isset($_GET['rayonP'])) {
		$dari = $_GET['dari'];
		$sampai = $_GET['sampai'];

		$where = "WHERE kode_rayon = '$kd_rayon' AND tgl_kejadian BETWEEN '$dari' AND '$sampai'";
		$sum = "nis,nama,rombel,kode_rayon,SUM(skor_r) as skor_reward, SUM(skor_p) as skor_punishment";
		$data = $aksi->tampil_sum($sum,$table,$where,"GROUP BY nis ORDER BY nis ASC");
		$nama = "Laporan Kinerja Siswa Rayon ".$rayon['rayon']." Periode ".$dari." sampai ".$sampai.".pdf";
		...
		$sum = "nis,nama,rombel,kode_rayon,SUM(skor_r) as skor_reward, SUM(skor_p) as skor_punishment";
		$data = $aksi->tampil_sum($sum,$table,$where,"GROUP BY nis ORDER BY nis ASC");
		$nama = "Laporan Kinerja Siswa Rayon ".$rayon['rayon']." Semester ".$semester." tp ".$tpel.".pdf";
	}
?>
<!DOCTYPE html>
<html>
			...
			<table width="100%" border="1" cellspacing="0" cellpadding="3" >
				<thead>
			    	<tr>
			    		...
			    		<th width="6%">JK</th>
			    		<th width="10%"><center>Rombel</center></th>
