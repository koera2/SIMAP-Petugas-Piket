<?php  
	...
	if (isset($_GET['siswaM'])) {
		$bulan = $_GET['bulan'];
		$tahun = $_GET['tahun'];

		$kinerja = $aksi->sumdata("SUM(skor_p) as total_punishment,SUM(skor_r) as total_reward","tbl_kinerja_siswa WHERE nis = '$nis' AND MONTH(tgl_kejadian)='$bulan' AND YEAR(tgl_kejadian)='$tahun'"); 
		$where = "WHERE nis = '$nis' AND MONTH(tgl_kejadian)='$bulan' AND YEAR(tgl_kejadian)='$tahun'";
	...
	}elseif(isset($_GET['siswaP'])){
		$dari = $_GET['dari'];
		$sampai = $_GET['sampai'];

		$kinerja = $aksi->sumdata("SUM(skor_p) as total_punishment,SUM(skor_r) as total_reward","tbl_kinerja_siswa WHERE nis = '$nis' AND tgl_kejadian BETWEEN '$dari' AND '$sampai'"); 
		$where = "WHERE nis = '$nis' AND tgl_kejadian BETWEEN '$dari' AND '$sampai'";
		$sum = "kelompok_kinerja,kode_kinerja,skor_r,skor_p,tgl_kejadian,saksi,tahun_pelajaran,semester,MONTH(tgl_kejadian) as bulan,YEAR(tgl_kejadian) as tahun";
		$data = $aksi->tampil_sum($sum,$table,$where,"ORDER BY tahun ASC, semester ASC, bulan ASC, tgl_kejadian ASC");
	}elseif(isset($_GET['siswaS'])){
		$semester = $_GET['semester'];
		$tpel = $_GET['tp'];

		$kinerja = $aksi->sumdata("SUM(skor_p) as total_punishment,SUM(skor_r) as total_reward","tbl_kinerja_siswa WHERE nis = '$nis' AND semester='$semester' AND tahun_pelajaran = '$tpel'"); 
		$where = "WHERE nis = '$nis' AND semester='$semester' AND tahun_pelajaran = '$tpel'";
		$sum = "kelompok_kinerja,kode_kinerja,skor_r,skor_p,tgl_kejadian,saksi,tahun_pelajaran,semester,MONTH(tgl_kejadian) as bulan,YEAR(tgl_kejadian) as tahun";
		$data = $aksi->tampil_sum($sum,$table,$where,"ORDER BY tahun ASC, semester ASC, bulan ASC, tgl_kejadian ASC");
	}
...
<body onload="window.print()" >
	<div id="wrapper">
		...
		<div id="judul">
			<?php if (isset($_GET['siswaM'])) { ?>
				<h2 style="margin-bottom: 10px;margin-top: 10px;"><strong><?php echo  $judul." BULAN ";$aksi->bulan_kapital($bulan);echo " TAHUN ".$tahun; ?></strong></h4>
			<?php }elseif (isset($_GET['siswaP'])) {?>
				<h2 style="margin-bottom: 10px;margin-top: 10px;"><strong><?php echo  $judul." PERIODE ";$aksi->tanggal_kapital($dari);echo "  -  ";$aksi->tanggal_kapital($sampai) ?></strong></h4>
			<?php }else{?>
				<h2 style="margin-bottom: 10px;margin-top: 10px;"><strong><?php echo  $judul." SEMESTER ".$semester." TAHUN PELAJARAN ".$tpel; ?></strong></h4>
			<?php } ?>
		</div>
