<?php  
	...
	if (isset($_GET['rayonM'])) {
		...
		$nama = "Laporan Absensi Siswa Rayon ".$rayon['rayon']." Bulan ".$bulan." ".$tahun.".pdf";
	}elseif (isset($_GET['rayon'])) {
		$where = "WHERE kode_rayon = '$kd_rayon'";
		$sum = "nis,nama,kode_rayon,rombel,SUM(hadir) as jumlah_hadir,SUM(sakit) as jumlah_sakit,SUM(izin) as jumlah_izin,SUM(alpa) as jumlah_alpa,SUM(tugas) as jumlah_tugas";
		$data = $aksi->tampil_sum($sum,$table,$where,"GROUP BY nis ORDER BY nis ASC");
		$nama = "Laporan Absensi Siswa Rayon ".$rayon['rayon'].".pdf";
		...
		$sum = "nis,nama,kode_rayon,rombel,SUM(hadir) as jumlah_hadir,SUM(sakit) as jumlah_sakit,SUM(izin) as jumlah_izin,SUM(alpa) as jumlah_alpa,SUM(tugas) as jumlah_tugas";
		$data = $aksi->tampil_sum($sum,$table,$where,"GROUP BY nis ORDER BY nis ASC");
		$nama = "Laporan Absensi Siswa Rayon ".$rayon['rayon']." Periode ".$dari." sampai ".$sampai.".pdf";
		...
		$sum = "nis,nama,kode_rayon,rombel,SUM(hadir) as jumlah_hadir,SUM(sakit) as jumlah_sakit,SUM(izin) as jumlah_izin,SUM(alpa) as jumlah_alpa,SUM(tugas) as jumlah_tugas";
		$data = $aksi->tampil_sum($sum,$table,$where,"GROUP BY nis ORDER BY nis ASC");
		$nama = "Laporan Absensi Siswa Rayon ".$rayon['rayon']." Semester ".$semester." tp ".$tpel.".pdf";
	}
?>
<!DOCTYPE html>
<html>
			...
			<table width="100%" border="1" cellspacing="0" cellpadding="3" >
				<thead>
			    	<tr>
			    		...
			    		<th width="5%">JK</th>
			    		<th width="11%"><center>Rombel</center></th>
